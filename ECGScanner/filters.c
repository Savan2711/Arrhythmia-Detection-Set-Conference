...
int filter(int value){
	...
	//printf("GLOBAL_TEST : %d", GLOBAL_TEST);
	//GLOBAL_TEST = 1;
...
#pragma omp section
			{
				run_TestFilter((Test*)& TEST_DER, "DER", "../Testfiles/x_der.txt", der[GLOBAL_COUNT % MAX_DER]);		    //TEST DERIVATIVE FILTER
			}
#pragma omp section
			{
				run_TestFilter((Test*)& TEST_SQR, "SQR", "../Testfiles/x_sqr.txt", sqr[GLOBAL_COUNT % MAX_SQR]);			//TEST SQUARING FILTER
			}
#pragma omp section
			{
